import os
try:
    env_var = os.environ['MKL_NUM_THREADS']
...
def main():
    ...
    for inx in range(len(test_left_img)):
        ...
        # save to h5py
        with h5py.File('%s/%s/disp.h5'% (args.outdir, idxname.split('/')[0]), "w") as f:
            dset = f.create_dataset("data", data=pred_disp)
    ...
    if env_var is not None:
        os.environ['MKL_NUM_THREADS'] = env_var
